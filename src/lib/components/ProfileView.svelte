...
<section>
  ...
  {:else}
    ...
    <div class="meta">
      ...
      {/if}
      <div class="row">
        <!-- First in the row of what can be done here: following is the reason
             most visits to somebody else's page end, and unlike the icons
             beside it, it keeps a caption because it carries state. -->
        {#if !isSelf}
          <FollowButton {hex} />
        {/if}
        {#if npub}
