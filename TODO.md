...
- [ ] **ブラウザ内リレーの上流に write リレーを含める**
  ...
  - ※ **kind 3 だけは回避済み**。フォローリストは失うと全フォローが消えるので、
    `signAndPublish(ev, { writeRelays: true })` でブラウザ内リレーに加えて
...
- [ ] **kind 0 / kind 3 を自前で取得する薄いクライアント**
  - ※ **kind 3 の取得経路は入った**（`src/lib/contactsFetch.ts`）。ただしフォロー機能の
    ベース取得専用で、意図的に**ブラウザ内キャッシュを通さず上流リレーへ直接** REQ を投げる
    （kind 3 の鮮度をウィジェット側に任せられないため）。読み物として使い回すなら、
    キャッシュ経由の薄い版を別に足すことになる。kind 0 は未着手。
...
- [ ] **フォロー一覧 / フォロワー一覧**
  - フォローは相手の kind 3 の p タグを `nostr-profile` で並べるだけ。読むだけなので publish 不要。
    `fetchContacts` + `followedPubkeys` がそのまま使える（自分以外の pubkey も渡せる）。
...
- [x] **フォロー / フォロー解除**（対応済み）
  - ユーザー詳細画面のボタン（`src/lib/components/FollowButton.svelte`）。難所は UI ではなく
    **kind 3 が全置換**であることで、取りこぼしたまま publish すると相手のフォローを消す。
    そのためのガードが実装の大半を占める。
  - ベースは**押すたびに**上流リレーへ直接 REQ を投げて集め、`created_at` 最大を採る
    （`src/lib/contactsFetch.ts`）。ブラウザ内キャッシュを通さないのは、kind 3 をどれだけ
    保持するかを combine 側から指定できないため。表示（フォロー中バッジ）は 1 セッション
    1 回のキャッシュで足りるが、**書き込みのベースだけは毎回取り直す**。
  - **1 本も EOSE が返らなければ publish しない**。「誰も答えなかった」と「全員が無いと答えた」を
    区別するのが要で、後者（2 本以上が EOSE）のときだけ確認を出して新規リストを作る。
    1 本を条件にしないのは、NIP-42 AUTH 必須のリレーが空の EOSE を返すため。
  - 組み立ては純関数（`src/lib/contacts.ts`）。`content`（旧 NIP-02 のリレー JSON）と
    p 以外のタグ、relay hint / petname を保ち、`created_at` は `max(now, base + 1)`。
    署名前に `checkContactsDiff` が「増減はちょうど ±1 人か」「他の人が消えていないか」
    「content とタグが保たれているか」を検算し、通らなければ中止して理由をログに残す。
  - 直近に自分が publish したリストを sessionStorage に持ち、リレーの反映が遅れても
    連続操作で前の変更を巻き戻さない。二重実行は `pending` で禁止。
  - 送り先はブラウザ内リレー＋本人の write リレー（上記「ブラウザ内リレーの上流」参照）。
  - フォロー後は「フォロー中」タイムラインを `reloadKey` で作り直す（要素は kind 3 を
    組み立て時に一度しか読まない）。
  - 残り: フォロー数の表示、投稿カードからのフォロー、フォロー一覧画面。
